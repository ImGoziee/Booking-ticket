        ...
        >
            <div className="relative p-1 h-full">
                ...
                <div className="absolute z-50 top-0 flex flex-col items-center justify-end h-full w-full py-28">
                    <h2 className='text-3xl font-medium'>{isLogin ? "Reconnect with Us" : "Get Started with Us"}</h2>
                    <p className='text-sm mt-2 max-w-[17rem] text-center text-gray-400'>{ isLogin ? 'Securely sign in to manage your account and stay connected with us.' : 'Join us today and explore an exciting world of concerts and events.'}</p>
                    <button
                        type='button'
                        onClick={handleClick}
                        className={`mt-7 rounded-lg bg-white py-3 text-sm font-medium text-[#171717] transition duration-150 ease-in-out hover:bg-gray-200 w-2/5 ${process ? "opacity-50" : ""}`}
                        disabled={processing}
                    >
